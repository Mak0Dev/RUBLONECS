		...
		<div class="row">
					...
					<div class="col-12 col-lg-4">
						<div class="card">
							<div class="card card-body card-header">
								<h3 class="mb-0">
									{info ? info.data.username : 'Loading...'}
									{#if info.data.verified}
										<img src="/verified.svg" alt="Verified" class="inline-block w-4 h-4 ml-1" />
									{/if}
								</h3>
							</div>
							<div class="card-body">
			...
			<div class="col-12 col-md-6 col-lg-3 actions">
				...
				{#if rank.is("admin")}
					<Permission p="DeleteUser">
						<button
							...
							on:click={(e) => {
								e.preventDefault();
								modalBody = 'Confirm complete account deletion. All items will be transferred to BadDecisions, all other uploaded content (status, desc, forum posts, comments, etc) will be scrubbed. Username will be changed to "RUBLON_user_userId". Password will be reset. Cannot be reveresed.';
								modalCb = (t) => {
									if (t) {
										// null
										request
				...
				<Permission p="GiveUserBadge">
					...
					>
						<img src="/verified.svg" alt="Verified" class="inline-block w-3 h-3 mr-1" />
				...
				<Permission p="ResetDescription">
					<button
						...
						on:click={(e) => {
							e.preventDefault();
							modalBody = `The password for this account will be set to "changeme".
							Please make sure the user is active before resetting their password so they can quickly change it.
							Please also make sure you have enough proof that the user owned that account at any point.`;
							modalCb = (t) => {
								if (t) {
									request
