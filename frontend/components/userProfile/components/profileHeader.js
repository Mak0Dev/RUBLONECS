...
const ProfileHeader = props => {
  ...
  useEffect(() => {
    ...
    */
    ...
    buttons.push({
      name: 'Collectibles',
      url: `https://collectibles.karblox.cc?user=${store.userId}`,
    });
    if (!isOwnProfile) {
      buttons.push({
        name: 'Trade',
  ...
  const BcIcon = () => {
    if (bcLevel === 0 && !store.isVerified) {
	  ...
	  return (
		<>
		  {store.isVerified && <img src="/verified.svg" alt="Verified" style={{width: '18px', height: '18px', marginRight: '5px', verticalAlign: 'middle'}} />}
