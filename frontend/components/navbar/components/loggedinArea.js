...
const useDropdownStyles = createUseStyles({
  wrapper: {
    ...
    userSelect: 'none',
    background: 'var(--custom-surface, white)',
  },
  text: {
    padding: '10px',
    marginBottom: 0,
    fontSize: '16px',
    '&:hover': {
      background: 'var(--custom-surface, #eaeaea)',
      borderLeft: '4px solid var(--custom-primary, #FFAA5A)',
    },
    '&:hover > a': {
      marginLeft: '-4px',
    },
      ...
      <div className={`col-12 ${s.linkContainer}`}>
        <p className={s.text} title={authStore.robux.toLocaleString()}>
          <Link href='/My/Money.aspx'>
            <a>
              <span className='icon-nav-robux custom-currency-icon'/>
            </a>
          </Link>
        </p>
        ...
        <>
          <p className={s.text} title={authStore.tix.toLocaleString()}>
            <Link href='/My/Money.aspx'>
              <a>
                <span className='icon-nav-tix custom-currency-icon'/>
              </a>
            </Link>
          </p>
        ...
        <p className={s.text}>
          <a onClick={(e) => {
            ...
            <span className={`icon-nav-settings custom-settings-icon ${s.settingsIcon}`} id="nav-settings"/>
          </a>
        </p>
