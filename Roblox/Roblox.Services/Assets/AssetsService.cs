...
public class AssetsService : ServiceBase, IService
{
    ...
    private async Task CreatePackageThumbnail(long assetId, CancellationToken? cancellationToken = null)
    {
        ...
        // Do not send placeholder IDs (-1) to the renderer. The old package
        // thumbnail code always appended PackageShirtAssetId/PackagePantsAssetId,
        // whose default value is -1. RCC then requested /asset?id=-1 and the
        // website correctly returned "Asset is invalid or does not exist".
        if (Configuration.PackageShirtAssetId > 0)
        {
            renderAssets.Add(new AvatarAssetEntry
            {
                id = Configuration.PackageShirtAssetId,
                assetType = new AvatarAssetTypeEntry { id = (int)Type.Shirt },
            });
        }
        if (Configuration.PackagePantsAssetId > 0)
        {
            renderAssets.Add(new AvatarAssetEntry
            {
                id = Configuration.PackagePantsAssetId,
                assetType = new AvatarAssetTypeEntry { id = (int)Type.Pants },
            });
        }
