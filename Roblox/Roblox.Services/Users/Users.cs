...
public class UsersService : ServiceBase, IService
{
    ...
    public async Task<IEnumerable<MultiGetEntry>> MultiGetUsersById(IEnumerable<long> userIds)
    {
        ...
        var sql = new SqlBuilder();
        var t = sql.AddTemplate("SELECT u.id, u.username as name, u.username as displayName, COALESCE((SELECT ja.is_verified FROM join_application ja WHERE ja.user_id = u.id ORDER BY ja.updated_at DESC NULLS LAST LIMIT 1), false) as isVerified FROM \"user\" u /**where**/");
        sql.OrWhereMulti("u.id = $1", ids);
    ...
    {
        // Gender is no longer collected by the public signup page. Keep a stable
        // default for the legacy user_settings schema.
        return CreateUser(username, password, Gender.Male);
    }
