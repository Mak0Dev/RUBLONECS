...
public class GamesService : ServiceBase, IService
{
    ...
    public async Task<IEnumerable<PlaceEntry>> MultiGetPlaceDetails(IEnumerable<long> placeIds)
    {
        ...
        // Revival places are playable when the place record exists. The legacy Roblox
        // client expects an explicit boolean; leaving it at C#'s default false makes the
        // website display "You cannot access this place at this time." even for valid places.
        foreach (var place in result)
        {
            // Revival places do not use Roblox's moderation/access gate.
            // A place returned by the database is playable unless the join
            // allocator itself reports a failure.
