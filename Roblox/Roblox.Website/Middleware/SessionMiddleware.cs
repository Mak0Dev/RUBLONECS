...
public class SessionMiddleware
{
    ...
    public static void Configure(string newJwtKey)
    {
        ...
        if (string.IsNullOrWhiteSpace(newJwtKey))
            throw new InvalidOperationException("Jwt:Sessions is missing or empty.");
    ...
    {
        if (string.IsNullOrWhiteSpace(cookieJwtKey))
            throw new InvalidOperationException("Session JWT key has not been configured.");

        var token = Encoder.Encode(obj, cookieJwtKey);
        if (string.IsNullOrWhiteSpace(token)) throw new NullReferenceException();
        return token;
    }
    ...
    public static T DecodeJwt<T>(string token)
    {
        ...
        if (string.IsNullOrWhiteSpace(json)) throw new NullReferenceException();
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        try
        {
            if (ctx.Request.Cookies.ContainsKey(CookieName))
            {
                ...
                if (!string.IsNullOrEmpty(decodedResult.sessionId))
                {
                    ...
                    // Accounts are no longer gated by join_application/invite status.
        ...
        catch (System.Exception e) when (e is InvalidTokenPartsException
            or NullReferenceException
            or FormatException
            or SignatureVerificationException)
        {
            // Invalid/expired cookie must never become an authenticated session.
