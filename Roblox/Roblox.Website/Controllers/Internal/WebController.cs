...
public class WebController : ControllerBase
{
    ...
    {
        // Game launching must work in both Debug/Build and Release configurations.
        // The old RELEASE-only guard caused /game/get-join-script to return HTTP 500
        // when the website was started with `dotnet run --configuration Build`.
        if (placeId <= 0)
        ...
        // Revivals do not use Roblox moderation state as a client-side playability
        // gate. The place existing as a Place asset is sufficient here; the game
        // server performs the actual availability check.
        var ipAddress = GetIP();
        ...
        var joinScriptUrl = Configuration.BaseUrl + "/placelauncher.ashx?ticket=" + encodedTicket;
        var args =
            $"--authenticationUrl {Roblox.Configuration.BaseUrl}/Login/Negotiate.ashx --authenticationTicket {ticket} --joinScriptUrl {joinScriptUrl}";
        ...
        return new
        {
            ...
            joinScriptUrl,
            retroArgs = args,
            clientArgs = $"join?place={placeId}&ticket={encodedTicket}",
            placeId,
        };
    }
