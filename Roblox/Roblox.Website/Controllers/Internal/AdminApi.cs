...
public class AdminApiController : ControllerBase
{
    ...
    public async Task<dynamic> GetUserInfoDetailed(long userId)
    {
        ...
        result.verified = (object)(joinApp?.isVerified ?? false);
        result.year = year.ToString();
        return result;
    }

    [HttpPost("users/verify"), StaffFilter(Access.GiveUserBadge)]
    public async Task<dynamic> VerifyUser([Required, FromBody] UserIdRequest request)
    {
        var user = await services.users.GetUserById(request.userId);
        if (user.IsDeleted())
            throw new StaffException("Deleted users cannot be verified");

        var application = await services.users.GetApplicationByUserId(request.userId);
        if (application == null)
            throw new StaffException("This user does not have a join application to verify");

        var newValue = !application.isVerified;
        await db.ExecuteAsync(
            "UPDATE join_application SET is_verified = :is_verified, updated_at = now() WHERE user_id = :user_id",
            new { is_verified = newValue, user_id = request.userId });

        return new { verified = newValue };
    }
...
-The Roblox Team"
    ...
    [HttpPost("asset/copy-ugc"), StaffFilter(Access.MigrateAssetFromRoblox)]
    public async Task<dynamic> CopyUgcFromRoblox([Required, FromForm] CopyUgcRequest request)
    {
        if (request.OBJ == null)
            throw new StaffException("No OBJ file specified");
        if (string.IsNullOrWhiteSpace(request.rbxURL))
            throw new StaffException("Roblox URL is required");

        var robloxAssetId = Libraries.Assets.UrlUtilities.GetAssetIdFromUrl(request.rbxURL);

        try
        {
            var existingAssetId = await services.assets.GetAssetIdFromRobloxAssetId(robloxAssetId);
            return new { assetId = existingAssetId, meshId = existingAssetId };
        }
        catch (RecordNotFoundException)
        {
        ...
        if (string.IsNullOrWhiteSpace(details.Name))
            throw new StaffException("Roblox asset has no name");

        await using var obj = request.OBJ.OpenReadStream();
        if (obj.Length == 0)
            throw new StaffException("OBJ file is empty");
        obj.Position = 0;

        // The uploaded OBJ is the actual mesh payload. Keep the Roblox ID on the
        // asset so future copies can be de-duplicated even when Roblox changes
        // how the source asset is delivered.
        ...
        return new { assetId = assetDetails.assetId, meshId = assetDetails.assetId };
    }
    ...
    {
        if (bundleId <= 0)
            throw new StaffException("Invalid bundle ID");

        // BodyParts bundles must be copied as individual assets first.  Do not use
        // MigrateItemFromRoblox here: bundle import needs the same path as the
        // original bundle importer (GetProductInfo + GetAssetContent + CreateAsset),
        // because AssetDelivery V2 is rate-limited and can return incomplete metadata.
        var details = await services.robloxApi.GetBundle(bundleId);
        if (!string.Equals(details.bundleType, "BodyParts", StringComparison.OrdinalIgnoreCase))
            throw new StaffException("Invalid bundleType " + details.bundleType + ". Only BodyParts bundles are supported.");

        if (details.items == null || details.items.Count == 0)
            throw new StaffException("Roblox returned no bundle items.");

        // Do not silently reuse an old/empty Package. The operation below is
        // intentionally idempotent at the asset level and repairs an incomplete
        // package by importing its missing children.
        var existingPackages = await services.assets.SearchCatalog(new CatalogSearchRequest
        {
            limit = 50,
            include18Plus = true,
            includeNotForSale = true,
        ...
        long existingPackageId = 0;
        if (existingPackages._total > 0 && existingPackages.data != null)
        {
            var existingInfo = await services.assets.MultiGetInfoById(existingPackages.data.Select(x => x.id));
            var package = existingInfo.FirstOrDefault(x =>
                x.assetType == Type.Package &&
                string.Equals(x.name, details.name, StringComparison.Ordinal));
            if (package != null)
        ...
        foreach (var item in details.items)
        {
            ...
            try
            {
                Console.WriteLine("[BundleImport] Getting asset " + item.id + " (" + item.name + ")");

                // Marketplace product info is deliberately used instead of
                // AssetDeliveryV2 for bundle entries.
                var info = await services.robloxApi.GetProductInfo(item.id, false);
                if (info.AssetTypeId == null)
                {
                    errors.Add(item.id + ": Roblox returned no AssetTypeId");
                    continue;
                }

                var allowed = new[]
                {
                    Type.Head, Type.Torso,
                    Type.RightArm, Type.LeftArm,
                    Type.RightLeg, Type.LeftLeg,
                    Type.Shirt, Type.Pants, Type.TeeShirt,
                    Type.Face,
                    Type.Hat, Type.HairAccessory,
                    Type.FrontAccessory, Type.BackAccessory,
                    Type.WaistAccessory, Type.NeckAccessory,
                    Type.ShoulderAccessory, Type.FaceAccessory,
                    Type.Gear,
                };

                if (!allowed.Contains(info.AssetTypeId.Value))
                {
                    Console.WriteLine("[BundleImport] Skipping unsupported asset " + item.id +
                                      " type=" + info.AssetTypeId.Value);
                    continue;
                }
                ...
                if (!isOk)
                {
                    errors.Add(item.id + ": asset validation failed for " + info.AssetTypeId.Value);
                    continue;
                }

                content.Position = 0;

                // Keep the Roblox asset ID as roblox_asset_id for future
                // deduplication, but let the database allocate our local ID.
                var created = await services.assets.CreateAsset(
                    string.IsNullOrWhiteSpace(item.name) ? (info.Name ?? ("Asset" + item.id)) : item.name,
                    info.Description,
                    1,
                    CreatorType.User,
                    1,
                    content,
                    info.AssetTypeId.Value,
                    Genre.All,
                    ModerationStatus.ReviewApproved,
                ...
                ids.Add(created.assetId);
                Console.WriteLine("[BundleImport] Imported Roblox " + item.id +
                                  " -> local " + created.assetId + " type=" + info.AssetTypeId.Value);
            }
            catch (Exception e)
            {
                errors.Add(item.id + ": " + e.Message);
                Console.WriteLine("[BundleImport] Failed asset " + item.id + ": " + e);
            }
        }

        ids = ids.Distinct().ToList();

        // If a previous attempt already imported some of these Roblox assets,
        // include their local IDs as well. This repairs partially-created packages.
        foreach (var item in details.items.Where(x => string.Equals(x.type, "Asset", StringComparison.OrdinalIgnoreCase)))
        {
            try
            {
                var localId = await services.assets.GetAssetIdFromRobloxAssetId(item.id);
                if (localId > 0)
                    ids.Add(localId);
            }
            catch (RecordNotFoundException)
        ...
        if (ids.Count == 0)
        {
            var detail = errors.Count == 0
                ? "No asset entries were returned by Roblox."
                : string.Join("; ", errors.Take(8));
            throw new StaffException("The Roblox BodyParts bundle could not be imported. " + detail);
        }

        long packageAssetId;
        if (existingPackageId != 0)
        {
            packageAssetId = existingPackageId;
        }
        else
        {
            var package = await services.assets.CreateAsset(
                details.name,
                details.description,
                1,
                CreatorType.User,
                ...
                bundleId,
                disableRender: true);
            packageAssetId = package.assetId;
        }

        // Package validation in the admin CreateAsset path is intentionally
        // reused here so only valid body/accessory children enter the package.
        var currentChildren = (await services.assets.GetPackageAssets(packageAssetId)).ToHashSet();
        foreach (var localId in ids)
        {
            if (localId <= 0 || currentChildren.Contains(localId))
                continue;

            var childInfo = await services.assets.GetAssetCatalogInfo(localId);
            ...
            if (!packageTypes.Contains(childInfo.assetType))
                continue;

            await services.assets.InsertPackageAsset(packageAssetId, localId);
            currentChildren.Add(localId);
        }

        if (currentChildren.Count == 0)
            throw new StaffException("Package was created but no valid bundle parts could be attached.");

        services.assets.RenderAsset(packageAssetId, Type.Package);

        return new
        {
            assetId = packageAssetId,
            assetIds = currentChildren.ToArray(),
            importedCount = currentChildren.Count,
            warnings = errors,
        };
    }

    [HttpPost("bundle/copy-animation-bundle"), StaffFilter(Access.CreateBundleCopiedFromRoblox)]
    public async Task<dynamic> CopyAnimationBundleFromRoblox([FromQuery] long bundleId)
    {
        if (bundleId <= 0)
            throw new StaffException("Invalid bundle ID");

        var bundle = await services.robloxApi.GetBundle(bundleId);
        var animationTypes = new[]
        {
            Type.Animation, Type.ClimbAnimation, Type.DeathAnimation, Type.FallAnimation,
            Type.IdleAnimation, Type.JumpAnimation, Type.RunAnimation, Type.SwimAnimation,
            Type.WalkAnimation, Type.PoseAnimation,
        };

        var copiedAssetIds = new List<long>();
        foreach (var item in bundle.items ?? new())
        {
            try
            {
                var migrated = await MigrateItem.MigrateItemFromRoblox(
                    $"https://www.roblox.com/catalog/{item.id}/-",
                    false,
                    null,
                    animationTypes,
                    autoApprove: true);
                copiedAssetIds.Add(migrated.assetId);
            }
            catch (AssetTypeNotAllowedException)
            {
                // Ignore non-animation entries; some Roblox bundle responses
                // include costume metadata alongside the animation assets.
            }
        }

        copiedAssetIds = copiedAssetIds.Distinct().ToList();
        return new { assetIds = copiedAssetIds };
    }
