...
public class MigrateItem
{
        ...
        int? price = null, IEnumerable<Models.Assets.Type>? allowedTypes = null, ProductDataResponse? defaultResponse = null, bool doRender = true, bool autoApprove = false)
    {
        ...
        // AssetDelivery V2 is aggressively rate-limited by Roblox.  Bundle imports
        // commonly hit HTTP 429 after a few items, so do not make migration depend
        // exclusively on the V2 endpoint.  Fall back to the marketplace/html
        // product-info path; content is fetched later through GetAssetContent().
        ProductDataResponse robloxDetails;
        if (defaultResponse != null)
        {
            robloxDetails = defaultResponse;
        }
        else
        {
            try
            {
                robloxDetails = await robloxApi.GetProductInfoAssetDelivery(assetId);
            }
            catch (Exception e) when (e.Message.Contains("TooManyRequests", StringComparison.OrdinalIgnoreCase)
                                     || e.Message.Contains("429", StringComparison.OrdinalIgnoreCase))
            {
                Console.WriteLine($"[RobloxApi] AssetDelivery V2 rate limited asset {assetId}; falling back to product info/content API.");
                robloxDetails = await robloxApi.GetProductInfo(assetId);
            }
        }
