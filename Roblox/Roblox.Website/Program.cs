...
// Karblox 2017 client protocol in every build configuration.
Roblox.Configuration.RobloxAppPrefix = "rbxeconsim:";
...
{
    FileProvider = new PhysicalFileProvider(Path.Combine(builder.Environment.ContentRootPath, "LegacySignupAssets")),
    RequestPath = "/legacy-signup-assets",
...
app.UseStaticFiles(new StaticFileOptions
{
    FileProvider = new PhysicalFileProvider(Path.Combine(Roblox.Configuration.PublicDirectory, "UnsecuredContent")),
...
app.UseStaticFiles(new StaticFileOptions
{
    FileProvider = new PhysicalFileProvider(Path.Combine(Roblox.Configuration.PublicDirectory, "img")),
    RequestPath = "/img",
    OnPrepareResponse = prepareResponseForCache,
});
