...
public class RobloxApi
{
    ...
    public async Task<BundleResponseEntry> GetBundle(long bundleId)
    {
        var urls = new[]
        {
            ...
            "https://catalog.roblox.com/v1/bundles/details?bundleIds=" + bundleId,
        };
        ...
        foreach (var url in urls)
        {
            try
            {
                ...
                if (!result.IsSuccessStatusCode)
                {
                    lastError = new Exception("GetBundle error: " + bundleId + " " + result.StatusCode);
                    continue;
                }

                var json = JsonSerializer.Deserialize<List<BundleResponseEntry>>(
                    await result.Content.ReadAsStringAsync(cancel.Token));

                if (json == null)
                {
                    lastError = new Exception("Null bundle response");
                    continue;
                }

                var entry = json.FirstOrDefault(x => x.id == bundleId);
                if (entry == null)
                {
                    lastError = new Exception("No bundle matches this id");
                    continue;
                }
        ...
        throw lastError ?? new Exception("Unable to retrieve bundle " + bundleId);
    }
